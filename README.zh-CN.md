...
这是一个你的项目应该遵循的垃圾代码书写准则的列表，称之为适当的垃圾代码。
...
"state-of-the-art shitcode" 徽章的源代码:
...
### 💩 永远使用 `flag` 命名 Boolean 值

为您的同事留下思考 Boolean 值的含义的时间
...
不要把程序逻辑分成可读的部分。如果 IDE 的搜索停止，而您无法找到所需的文件或函数，这该怎么办?

- 一个文件中 10000 行代码是 OK 的。
- 一个函数体有 1000 行代码是 OK 的。
- 在一个' service.js ' 中处理许多服务(第三方库和内部库、一些工具、手写的数据库 ORM 和 jQuery 滑块)? 这是 OK 的。
